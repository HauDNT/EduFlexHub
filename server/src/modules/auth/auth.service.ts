...
export class AuthService {
  ...
  async userLogin(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        'Xảy ra lỗi từ phía server trong quá trình đăng nhập',
      );
    }
  }
  ...
  async accessWithGoogle(
    ...
    try {
      ...
      if (existsUser !== null) {
        if (option === 'login') {
          const updateSocialAccountInfo = await this.updateLoginSocialAccount(
            existsUser.id,
            parseUser,
          );
          ...
          if (updateSocialAccountInfo) {
            ...
            await this.sessionService.handleSessionsWhenLogin(
              userInfo,
              userAgent,
              ip,
            );
        ...
        if (option === 'register') {
          const accountRole = await this.roleRepository.findOneBy({
            id: RoleEnum.Student,
          });
          const createSocialAccountInfo =
      ...
      console.log(
        '--> Xảy ra lỗi trong quá trình đăng nhập/ký với tài khoản Google: ',
        e,
      );
      ...
      throw new InternalServerErrorException(
        'Đã xảy ra lỗi ở phía server trong quá trình đăng nhập/ký với tài khoản Google',
      );
    }
  }
  ...
  async savedNewSocialAccountData(parseJSONData: any): Promise<boolean> {
    ...
    try {
      ...
      const role = await this.roleRepository.findOneBy({
        id: RoleEnum.Student,
      }); // Mặc định tài khoản đăng nhập SSO là cho học viên
      ...
      if (parseJSONData) {
        ...
        const saveSocialAccount: SocialAccount = await queryRunner.manager.save(
          SocialAccount,
          {
  ...
  async updateLoginSocialAccount(
    existsUserId: number,
    socialData: any,
  ): Promise<boolean> {
    try {
      const updateResult = await this.socialAccountRepository.update(
        existsUserId,
        {
      ...
      console.log(
        '--> Xảy ra lỗi trong quá trình lưu phiên đăng nhập mới từ tài khoản Google: ',
        e,
      );
      throw new ExceptionHandler();
    }
  ...
  async logout(
    ...
    const deleteSession = await this.sessionService.deleteSession(
      user,
      userAgent,
      ip,
    );
