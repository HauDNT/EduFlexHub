...
import { useToast, usePaginate, useFetchResource, useRestoreResource, useForceDeleteResource } from '@/hooks';
...
export default function MemberRestore() {
  ...
  const handleForceDeleteUsers = useForceDeleteResource(
    'users',
    'userIds',
    () => {
      toast({
        title: `Đã xoá vĩnh viễn tài khoản người dùng thành công`,
        variant: 'success',
      })
    },
    (error) => {
      toast({
        title: 'Xoá vĩnh viễn tài khoản người dùng thất bại',
        description: handleAxiosError(error),
        variant: 'destructive',
      });
    },
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle={RoleEnum[+type].charAt(0).toUpperCase() + RoleEnum[+type].slice(1) + ' deleted'} />
      ...
      <div className='space-y-6'>
        <CustomTable
          ...
          handleDelete={async (userSelected) => handleForceDeleteUsers.mutateAsync(userSelected)}
